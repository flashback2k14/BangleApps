...
function showChangeLog(appid) {
  ...
  httpGet(`apps/${appid}/ChangeLog`)
    .then(show)
    .catch(() => show('No Change Log available'));
}
function showReadme(appid) {
  ...
  httpGet(appPath + app.readme)
    .then(show)
    .catch(() => show('Failed to load README.'));
}
...
function refreshLibrary() {
  ...
  if (currentSearch) {
    visibleApps = visibleApps.filter(
      (app) => app.name.toLowerCase().includes(currentSearch) || app.tags.includes(currentSearch)
    );
  }

  panelbody.innerHTML = visibleApps
    .map((app, idx) => {
    ...
    <div class="tile-icon">
      <figure class="avatar"><img src="apps/${app.icon ? `${app.id}/${app.icon}` : 'unknown.png'}" alt="${escapeHtml(
        app.name
      )}"></figure><br/>
    </div>
    <div class="tile-content">
      ...
      <a href="https://github.com/espruino/BangleApps/tree/master/apps/${
        app.id
      }" target="_blank" class="link-github"><img src="img/github-icon-sml.png" alt="See the code on GitHub"/></a>
    </div>
    <div class="tile-action">
      <button class="btn btn-link btn-action btn-lg ${!app.custom ? 'text-error' : 'd-hide'}" appid="${
        app.id
      }" title="Favorite"><i class="icon"></i>${favourite ? '&#x2665;' : '&#x2661;'}</button>
      <button class="btn btn-link btn-action btn-lg ${appInstalled && app.interface ? '' : 'd-hide'}" appid="${
        app.id
      }" title="Download data from app"><i class="icon icon-download"></i></button>
      <button class="btn btn-link btn-action btn-lg ${app.allow_emulator ? '' : 'd-hide'}" appid="${
        app.id
      }" title="Try in Emulator"><i class="icon icon-share"></i></button>
      <button class="btn btn-link btn-action btn-lg ${version.canUpdate ? '' : 'd-hide'}" appid="${
        app.id
      }" title="Update App"><i class="icon icon-refresh"></i></button>
      <button class="btn btn-link btn-action btn-lg ${!appInstalled && !app.custom ? '' : 'd-hide'}" appid="${
        app.id
      }" title="Upload App"><i class="icon icon-upload"></i></button>
      <button class="btn btn-link btn-action btn-lg ${appInstalled ? '' : 'd-hide'}" appid="${
        app.id
      }" title="Remove App"><i class="icon icon-delete"></i></button>
      <button class="btn btn-link btn-action btn-lg ${app.custom ? '' : 'd-hide'}" appid="${
        app.id
      }" title="Customise and Upload App"><i class="icon icon-menu"></i></button>
    </div>
  </div>
  `;
    })
...
function showLoadingIndicator(id) {
  ...
  // Loading indicator
  panelbody.innerHTML =
    '<div class="tile column col-12"><div class="tile-content" style="min-height:48px;"><div class="loading loading-lg"></div></div></div>';
}
...
function getAppsToUpdate() {
  ...
  appsInstalled.forEach((appInstalled) => {
    ...
    if (app.version != appInstalled.version) appsToUpdate.push(app);
  });
  return appsToUpdate;
}
...
function refreshMyApps() {
  ...
  panelbody.innerHTML = appsInstalled
    .map((appInstalled) => {
      var app = appNameToApp(appInstalled.id);
      var version = getVersionInfo(app, appInstalled);
      return `<div class="tile column col-6 col-sm-12 col-xs-12">
    ...
    <div class="tile-action">
      ...
      <button class="btn btn-link btn-action btn-lg" appid="${
        app.id
      }" title="Remove App"><i class="icon icon-delete"></i></button>
    </div>
  </div>
...
function installMultipleApps(appIds, promptName) {
  ...
  if (apps.some((x) => x === undefined)) return Promise.reject('Not all apps found');
  var appCount = apps.length;
  return showPrompt('Install Defaults', `Remove everything and install ${promptName} apps?`)
    .then(() => {
    ...
    .then(() => {
      ...
      return new Promise((resolve, reject) => {
        function upload() {
          ...
          Comms.uploadApp(app, 'skip_reset')
            .then((appJSON) => {
              Progress.hide({ sticky: true });
              if (appJSON) appsInstalled.push(appJSON);
...
htmlToArray(document.querySelectorAll('.btn.refresh')).map((button) =>
...
htmlToArray(document.querySelectorAll('.btn.updateapps')).map((button) =>
  button.addEventListener('click', () => {
    var appsToUpdate = getAppsToUpdate();
